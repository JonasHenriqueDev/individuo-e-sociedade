...
func _on_area_2d_area_entered(area: Area2D) -> void:
	if area.name == "Cabo2Area":
		...
		is_connected = true
		touch_index = -1
		position.x = x_clamp_max 
		
func _on_cabo_1_area_area_exited(area: Area2D) -> void:
	if area.name == "Cabo2Area":
		get_parent().get_node("On").visible = false
		get_parent().get_node("Off").visible = true
		
		# Novo: Permite arrastar novamente se sair
		is_connected = false
